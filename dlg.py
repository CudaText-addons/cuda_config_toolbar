...
class DialogButtons:
    ...
    def show(self):
        ...
        self.h_main = self.init()
        dlg_proc(self.h_main, DLG_CTL_PROP_SET, name='list', prop={'items': items, 'val': 0} )
        dlg_proc(self.h_main, DLG_CTL_FOCUS, name='list')
        dlg_proc(self.h_main, DLG_SHOW_MODAL)


    def callback_main_close(self, id_dlg, id_ctl, data='', info=''):
    ...
    def init(self):
        ...
        n=dlg_proc(h, DLG_CTL_ADD, 'button')
        dlg_proc(h, DLG_CTL_PROP_SET, index=n, prop={'name': 'btn_close',
          'x': 220,
          'w': 200,
          'y': 460,
          'cap': 'Close',
          'on_change': self.callback_main_close,
           } )
